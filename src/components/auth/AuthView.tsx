...
const GoogleBtn = ({ onClick, disabled }: any) => (
  <button
    type="button"
    onClick={onClick}
    disabled={disabled}
...
const InputField = ({ icon: Icon, type, name, value, onChange, placeholder, showToggle, onTogglePassword, showPassword }: any) => (
  <div className="relative">
    ...
    {showToggle && (
      <button
        ...
        className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors focus-ring"
...
export const AuthView = () => {
  ...
  useEffect(() => {
    // Check if there's a result from Google sign-in redirect
  ...
  const handleGoogleSignIn = async () => {
    ...
    setMessage(null);
    try {
      // Use signInWithRedirect instead of signInWithPopup to prevent 'popup-closed-by-user' errors
      // caused by mobile browsers, popup blockers, or Cross-Origin-Opener-Policy issues.
  ...
  if (showPrivacy) {
    return (
      <section
        ...
        className="absolute inset-0 z-[100] bg-white dark:bg-[#0A0A0A] overflow-y-auto animate-fadeInScale"
      >
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0A0A0A] text-gray-900 dark:text-gray-100 flex items-center justify-center p-4 pt-24 transition-colors duration-500 relative overflow-hidden">
      ...
      <button 
        ...
        className="absolute top-24 left-6 md:left-12 flex items-center space-x-2 text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 hover:text-[#109121] dark:hover:text-[#16C72E] transition-colors z-50 focus-ring"
        aria-label="Go back"
      >
        <ArrowLeft size={16} />
        <span>Back</span>
      ...
      <div className="hidden md:block relative w-full max-w-5xl h-[650px] bg-white dark:bg-[#111] rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-gray-100 dark:border-gray-800 overflow-hidden mx-auto">
        ...
        >
          ...
          <form onSubmit={handleAuth} className="space-y-5">
            {error && (
              <div role="alert" aria-live="assertive" className="flex items-center gap-2 text-red-500 text-sm text-center mb-2 animate-shake">
                <AlertCircle size={15} className="shrink-0" />
                <span>{error}</span>
              </div>
            )}
            {message && <div role="status" aria-live="polite" className="text-green-500 text-sm text-center mb-2">{message}</div>}
              ...
              <div>
                ...
                {!isUpdatePassword && (
                  <div className="mt-2 text-right">
                    <button type="button" onClick={() => window.location.hash = 'reset'} className="text-xs text-[#E8B93D] hover:underline font-semibold">Forgot Password?</button>
                  </div>
                )}
              </div>
            )}

            <button type="submit" disabled={loading} aria-busy={loading} className="w-full bg-[#109121] hover:bg-[#0a5f15] text-white py-3.5 rounded-2xl font-bold uppercase tracking-widest text-sm transition-all shadow-lg shadow-[#109121]/20 hover:shadow-[#109121]/40 focus-ring disabled:opacity-70">
              {loading ? <><span className="spinner" aria-hidden="true" />{isResetPassword ? 'Sending...' : isUpdatePassword ? 'Updating...' : 'Signing in...'}</> : isResetPassword ? 'Send Link' : isUpdatePassword ? 'Update' : 'Sign In'}
            </button>
          ...
          {isResetPassword && (
            <button onClick={() => window.location.hash = 'signin'} className="mt-6 text-sm text-[#E8B93D] hover:text-[#C99A26] font-semibold transition-colors text-center w-full focus-ring">Back to Sign In</button>
          )}
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleAuth} className="space-y-5">
            ...
            <InputField icon={Mail} type="email" name="email" value={email} onChange={(e: any) => setEmail(e.target.value)} placeholder="Email address" />
            <InputField icon={Lock} type={showPassword ? "text" : "password"} name="password" value={password} onChange={(e: any) => setPassword(e.target.value)} placeholder="Password" showToggle showPassword={showPassword} onTogglePassword={() => setShowPassword(!showPassword)} />

            <div className="flex items-start mt-2 mb-4 bg-gray-50 dark:bg-white/5 p-3 rounded-xl border border-gray-200 dark:border-gray-600">
              <div className="flex items-center h-5">
                <input 
                  ...
                  className="w-5 h-5 border-2 border-gray-400 dark:border-gray-500 rounded bg-white focus:ring-2 focus:ring-[#109121] dark:bg-gray-700 accent-[#109121] cursor-pointer" 
              ...
              <label htmlFor="terms-desktop" className="ml-3 text-xs font-medium text-gray-600 dark:text-gray-300 leading-relaxed cursor-pointer select-none">
                I have read and agree to the <button type="button" onClick={() => { setShowPrivacy(true); window.scrollTo(0,0); }} className="text-[#109121] font-bold hover:underline dark:text-[#16C72E] py-1 px-0.5 focus-ring" aria-label="Read Privacy Policy">Privacy Policy</button> and <button type="button" onClick={() => { setShowTerms(true); window.scrollTo(0,0); }} className="text-[#109121] font-bold hover:underline dark:text-[#16C72E] py-1 px-0.5 focus-ring" aria-label="Read Terms and Conditions">Terms and Conditions</button>.
              </label>
            </div>

            <button type="submit" disabled={loading} aria-busy={loading} className="w-full bg-[#109121] hover:bg-[#0a5f15] text-white py-3.5 rounded-2xl font-bold uppercase tracking-widest text-sm transition-all shadow-lg shadow-[#109121]/20 hover:shadow-[#109121]/40 focus-ring disabled:opacity-70">
              {loading ? <><span className="spinner" aria-hidden="true" />Signing up...</> : 'Sign Up'}
            </button>
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {isSignUp ? (
              ...
              >
                <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center mb-8 shadow-[0_0_30px_rgba(255,255,255,0.2)] border-4 border-[#E8B93D]">
                  <img src="/samstones-logo.jpg" alt="Samstones Logo" className="w-20 h-20 rounded-full object-contain" />
                </div>
                <h2 className="text-4xl font-bold mb-4 font-serif text-[#E8B93D]">Welcome Back!</h2>
              ...
              >
                ...
                <h2 className="text-4xl font-bold mb-4 font-serif text-[#E8B93D]">Hello, Friend!</h2>
                <p className="mb-10 text-white/90 leading-relaxed font-medium px-4">
      ...
      <div className="md:hidden w-full max-w-md mx-auto">
        <div className="bg-white dark:bg-[#111] rounded-[2rem] shadow-xl border border-gray-100 dark:border-gray-800 p-8 flex flex-col items-center">
          ...
          <form onSubmit={handleAuth} className="space-y-4 w-full">
            ...
            {isSignUp && (
              <div className="flex items-start mt-2 mb-2 bg-gray-50 dark:bg-white/5 p-3 rounded-xl border border-gray-200 dark:border-gray-600">
                <div className="flex items-center h-5">
                  <input 
                    id="terms-mobile" 
                    type="checkbox" 
            ...
            <button type="submit" disabled={loading} aria-busy={loading} className="w-full mt-2 bg-[#109121] hover:bg-[#0a5f15] text-white py-3.5 rounded-2xl font-bold uppercase tracking-widest text-sm transition-all shadow-lg shadow-[#109121]/20 hover:shadow-[#109121]/40 focus-ring disabled:opacity-70">
              {loading ? <><span className="spinner" aria-hidden="true" />{isResetPassword ? 'Sending...' : isUpdatePassword ? 'Updating...' : isSignUp ? 'Signing up...' : 'Signing in...'}</> : isResetPassword ? 'Send Link' : isUpdatePassword ? 'Update' : isSignUp ? 'Sign Up' : 'Sign In'}
            </button>
          ...
          <div className="mt-8 text-center">
            {isResetPassword ? (
              <button onClick={() => window.location.hash = 'signin'} className="text-sm text-[#E8B93D] hover:text-[#C99A26] font-semibold transition-colors focus-ring">Back to Sign In</button>
            ) : (
              <button 
                onClick={() => window.location.hash = isSignUp ? 'signin' : 'signup'}
                className="text-sm font-semibold text-[#E8B93D] hover:text-[#C99A26] transition-colors focus-ring"
              >
